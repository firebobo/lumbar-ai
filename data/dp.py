...
class Dataset(torch.utils.data.Dataset):
    ...
    def loadImage(self, idx):
        ...
        return self.transforms(inp[np.newaxis, :, :]), heatmaps, np.array(labels).astype(np.float32)
    ...
    def show(self, heatmaps, inp, inp_img, kpt_change, kpt_change_pre):
        for ind, ks in enumerate(kpt_change):
            ...
            for jnd,k in enumerate(ks):
                if k[0] <= 0 or k[1] <= 0 or k[0] >= self.output_res*(2**(ind)) or k[1] >= self.output_res*(2**(ind)):
                    continue
